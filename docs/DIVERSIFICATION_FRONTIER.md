...
- **Sub-cent prices rounded to zero in the MILP objective WHEN THIS SWEEP RAN.** At generation time `sourcing.py` quantised unit prices to whole cents (`PRICE_SCALE = 100`), so an offer at $0.0031 entered the objective at $0.00. It was deliberately NOT changed for the sweep: every arm is the same MILP at the same resolution, so the comparison across k is internally consistent, and it remains so. **The code has since been fixed** (2026-08-28): a single `to_obj_units()` now carries every USD term at the objective's own milli-cent resolution, so the described behaviour is history, not current. These figures are therefore reproducible only against the pre-fix solver; re-running the sweep on current code is the way to get post-fix numbers. Measured, the fix moves 0 of 80 benchmark cells because no reference BOM contains a sub-cent part — so the frontier below is not expected to move materially, but it has not been re-measured and this file does not claim it has.
