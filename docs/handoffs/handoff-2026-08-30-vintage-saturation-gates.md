...
git log --oneline -1                      # expect 1c00994
git rev-list --count origin/main..main    # expect 0
git status --porcelain backend/seeds/data/   # expect empty

curl -s https://supply-chain-api-qy8x.onrender.com/version      # expect 1c00994...
curl -s https://supply-chain-ui-bhwz.onrender.com/version.json  # expect 1c00994...
```
...
- **Full backend suite: 1120 passed, 1 failed, 2 skipped** (`751 s`). The one failure is
...
### Verified live on `1c00994` (2026-08-30, end of session)

Nothing is outstanding from this session. Verified by direct calls, not by version hashes —
agreeing hashes did NOT catch the 500 earlier today:

| Check | Result |
|---|---|
| `POST /optimize/vrp`, out-of-stock cart | **400** "Insufficient stock … 140G-G2C3-C50 needs 4 but only 3 in stock" (was 500 "Solver failed") |
| `GET /newsvendor/evaluation` default | **200 in 0.18 s** (was 259.9 s) |
| same, `croston` / L=5 (former cache miss) | **200 in 0.06 s** (was ~107 s) |
| `?review_period_months=12` | **422** naming the bound (was a 500 traceback) |
| Full UI gate vs live site | **239 passed, 0 failed, 0 fatal** |

Full backend suite 1120 passed / 1 permitted failure / 2 skipped. ruff, mypy, `tsc -b --force`
and `npm run build` all clean. CI and Model CI green on `1c00994`.
...
#   expect 1120 passed, 1 failed, 2 skipped. ~12.5 min. The ONE permitted failure is
...
Nothing is blocking. Everything below is optional improvement.

1. **`STRESS_FRAME_MAX_AGE_DAYS = 120` turns the suite red on 2026-10-29** if nobody retrains.
   That is deliberate. The fix is to rerun `seeds/train_ml_models.py` and commit new artifacts —
   **not** to raise the constant. `python_version` provenance also starts populating on that
   retrain (the field is stamped but ungated).
2. **`docs/cvar_frontier.json` cannot be regenerated reproducibly** (backlog item 45). Its
   breadth arm runs a 15 s CP-SAT budget with 46/150 solves hitting it; two regenerations
   disagreed on the counters. No plan or cost moves. Now labelled a one-machine run log rather
   than a finding — the cheaper, non-rotting fix. Raising the time limit (~30 min + a 22 min
   regeneration) remains available if you ever want the counters stable instead of caveated.
3. **`docs/backend_verification.json` has no generator** and cannot be honestly pinned — it is a
   hand-run snapshot whose content is 42 live HTTPS responses with per-check `seconds`, so a pin
   would go red on a cold start and green on a broken build. Recorded, not faked.
4. **CartPage SKU display** (from item 11) and the **six caller-less `/market/*` routes** on
   public Swagger. The routes now answer honestly (`available: false` + `unavailable_reason`,
   never a fabricated `1.0`), but their upstream 404s and nothing consumes them — consider
   removing them from the public surface entirely.
5. **Chronos cross-platform determinism is unknown**, not disproven — torch never installs on CI,
   so it has never been exercised there. Its pin stays `slow` for that reason.
