...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - build_graph_state completes in < 2s and logs: "Graph built: {n_dist} distributors, {n_comp} components, {n_edges} offer edges, λ₂={fiedler:.4f} ({n_cc} connected components, {elapsed:.2f}s)"
...
def build_graph_state(db: Session) -> GraphState:
    ...
    logger.info(
        "Graph built: %d distributors, %d components, %d offer edges, "
        "λ₂=%.4f (%d connected components, %.2fs)",
        n_dist, n_comp, n_edges, fiedler, n_cc, elapsed,
    )
